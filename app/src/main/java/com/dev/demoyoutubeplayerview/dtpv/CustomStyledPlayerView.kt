...
open class CustomStyledPlayerView @JvmOverloads constructor(
    ...
    private val IGNORE_BORDER: Int = Utils.dpToPx(24)
    private val SCROLL_STEP: Int = Utils.dpToPx(16)
    private val SCROLL_STEP_SEEK: Int = Utils.dpToPx(8)
    ...
    private var mContext : CustomStyledPlayerView?=null
    fun clearIcon() {
...
//                break
    ...
    fun tap(): Boolean {
        if (PlayerActivity.locked) {
            Utils.showText(mContext!!, "", MESSAGE_TIMEOUT_LONG.toLong())
            setIconLock(true)
            return true
        }
    ...
    override fun onScroll(
        ...
        if (gestureOrientation == Orientation.HORIZONTAL || gestureOrientation == Orientation.UNKNOWN) {
            ...
            if (Math.abs(gestureScrollX) > SCROLL_STEP || gestureOrientation == Orientation.HORIZONTAL && Math.abs(
                ...
                var position: Long = 0
                val distanceDiff =
                    Math.max(0.5f, Math.min(Math.abs(Utils.pxToDp(distanceX) / 4), 10f))
                if (PlayerActivity.haveMedia) {
                    ...
                    var message: String = Utils.formatMilisSign(seekChange)
                    if (!isControllerFullyVisible) {
                        message += """
    ...
    override fun onScale(scaleGestureDetector: ScaleGestureDetector): Boolean {
        ...
        if (canScale) {
            ...
            restoreSurfaceView()
            clearIcon()
            setCustomErrorMessage((mScaleFactor*100).roundToInt().toString() + "%")
            return true
        }
        return false
    }
